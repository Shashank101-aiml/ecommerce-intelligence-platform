...
Part 1: a retail ETL pipeline (Python/Pandas + Airflow) that loads the UCI
Online Retail dataset into a PostgreSQL star schema, plus a Streamlit
dashboard for sales and retention analytics. Part 2 (churn prediction +
MLOps) is scaffolded under `ml/` and `api/` but not yet implemented.
...
for table definitions and validation rules.

## Prerequisites

- Docker Desktop (for Postgres + Airflow)
